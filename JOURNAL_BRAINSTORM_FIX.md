# Fixed: Journal Saving & Brainstorm LLM Issues âœ…

## Problems Identified & Fixed

### 1. **Journal Entries Not Being Saved** âŒ â†’ âœ…
...
**Files Fixed**:
- `src/lib/useAuth.tsx` â€” Now extracts and exposes `token` from Supabase session
- `src/pages/index.tsx` â€” Uses token from auth context instead of localStorage
- `src/pages/tasks.tsx` â€” Uses token from auth context instead of localStorage
- `src/pages/brainstorm.tsx` â€” Uses token from auth context instead of localStorage
...
### 2. **Brainstorm Using Heuristic Instead of LLM** âŒ â†’ âœ…
...
**Files Fixed**:
- `src/pages/api/brainstorm.ts` â€” Added auth wrapper and logging
...
1. Sign in with magic link (Supabase configured) âœ…
2. Write something in the journal textarea
3. Click "SAVE ENTRY"
4. **Should save immediately** âœ“
5. Check browser console (F12) â€” should NOT see auth errors
6. Check `data/db.json` â€” new entry should appear
...
**Example**:
- âœ… LLM: "Fix JWT token persistence issue in auth flow"
- âŒ Heuristic: "fix the auth bug", "write documentation", "deploy"

### Test 3: Check Logging

Open browser console (F12) â†’ Network tab:
1. When brainstorming, check `/api/brainstorm` response
2. Open browser console â†’ check for logs:
   ```
...
If you see `Using heuristic extraction` â†’ Groq failed or isn't configured.
...
â†’ Supabase auth not working. Follow `SUPABASE_SETUP.md` to configure redirect URLs.

### "Brainstorm using heuristic"
â†’ Check browser console logs. Likely causes:
...
â†’ Check browser console for error messages. Likely causes:
...
| `src/pages/index.tsx` | Use token from context | Journal entries now save âœ“ |
| `src/pages/tasks.tsx` | Use token from context | Tasks API now auth'd âœ“ |
| `src/pages/brainstorm.tsx` | Use token from context + auth | Brainstorm now requires login âœ“ |
| `src/pages/api/brainstorm.ts` | Added withAuth + logging | LLM now called with proper auth + debugging âœ“ |
